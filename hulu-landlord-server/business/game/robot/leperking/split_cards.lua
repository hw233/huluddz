		...
		if #cards == 4 then
			if #lepers > 0 then
				...
				table.insert(zhadan_list, {
					type = TYPE.zhadan,
					weight = COMB(v, ZHADAN.star_5.weight),
					cards = cards
				})
			else
				table.insert(zhadan_list, {
					type = TYPE.zhadan,
					weight = COMB(v, ZHADAN.yingzha.weight),
					cards = cards
				})
			end
			value_cards[v] = nil
...
local function split_cards(cards)
	...
	local lepers = util.remove_lepers(cards)
	local nlepers = #lepers
	local value_cards = util.get_value_cards(cards)
	
	-- 1. 双王(存在就当王炸), 一个的话就分配炸弹里凑一个5星炸
	if nlepers == 2 then
		table.insert(type_list, {type = TYPE.zhadan, weight = COMB(0, ZHADAN.wangzha.weight), cards = lepers})
		nlepers = 0
		lepers = {}
	end

	-- 2. 炸弹(保底2个)
	-- local leper = nlepers > 0 and lepers[1] or nil
	...
	-- 7.单牌
	dan_list = find_all_danX(value_cards, lepers)
