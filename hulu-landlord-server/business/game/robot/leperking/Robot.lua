...
local split_cards = require "game.robot.leperking.split_cards"
...
local function Robot(room, myid)
	...
	local function is_friend(pid)
		assert(pid ~= myid)
		if myid == room.landlord_id then
			return false
		else
			return pid ~= room.landlord_id
		end
	end
	...
	function self.playcard(playstatus)
		-- 跟据已拆好的牌, 从小到大. (飞机/三带 (带单/对/不带), 单牌, 对牌, 顺子, 连对, 炸弹)
		if playstatus == PlayCardState_DDZ.Normal then
			if last_play.friend then
				return after_friend_playcard(last_play.playedcards)
			else
				return after_enemy_playcard(last_play.playedcards)
			end
		else
			return {
				pass = false,
	...
	function self.p_playcard(pid, pass, playedcards)
		if not pass then
			last_play.pid = pid
			last_play.playedcards = playedcards
			if pid ~= myid then
				last_play.friend = is_friend(pid)
			else
				remove_from_handcards(playedcards)
				self.init_handcards(handcards)
			end
		end
	end

	function self.determine_landlord(landlord_id, bottom_cards)
		room.landlord_id = landlord_id
		room.bottom_cards = bottom_cards
		if landlord_id == myid then
			table.append(handcards, bottom_cards)
