...
local function listen_tcp(address, port, MSG)
    ...
    -- print("!!!! anysocket register_protocol")
...
local function close_websocket(fd)
    websocket.close(fd)
end
...
function M.init(protocol)
    M.protocol = protocol
    print("=== init anysocket is protocol ",M.protocol)
    if M.is_websocket() then
        skynet.error("=== anysocket is websocket====")
