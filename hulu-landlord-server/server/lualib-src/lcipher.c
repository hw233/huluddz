...
static void init(lua_State* L, struct evp_cipher_ctx* ctx_p, const unsigned char* key,
 int keylen, const unsigned char* iv, int ivlen, bool no_padding, bool is_encrypt){
    ...
    if (iv == NULL || ivlen != ctx_p->iv_size)
    {
        luaL_error(L, "cipher:init: incorrect iv size, expect %d", ctx_p->iv_size);
    }
...
luaopen_lcipher_c(lua_State* L) {
    if(!CIPHER_IS_INIT) {
        luaL_error(L, "ltls need init, Put enablecipher = true in you config file.");
    }
    luaL_Reg l[] = {
        {"new", lnew},
        {NULL, NULL},
