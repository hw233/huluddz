...
--user 登录用户
local my_id = "1000002"  --两个id = name 要统一
local user_name = "?qq1"
...
local function login()	
	...
	local ret = crypt.base64decode(string.sub(result, 7))
...
local heartbeatApiName = "heartbeat"
local heartbeatSessionCache = {}
...
local function send_request(name, args)
	...
	if name == heartbeatApiName then
		heartbeatSessionCache[session] = true
	else
...
local function print_request(name, args)
	...
	print("REQUEST", name, table.tostr(args))
	--handler对局事件
...
local function print_response(session, args)
	if heartbeatSessionCache[session] then
		return
	end

	print("RESPONSE", session, table.tostr(args))
	-- if args then
	-- 	for k,v in pairs(args) do
	-- 		print(k,v)
	-- 	end
...
while true do
	...
	if input_code then
			...
			---点赞 好友
			elseif cmd == "praise" then
				print("ssw_praise : ",inputArray[2])
				send_request("ssw_praise",{pid = inputArray[2]})
			---房间类型信息
			elseif cmd == "run" then
				local args = inputArray[3]
				if args then
                    local ok, ret = pcall(load("return " .. args))
                    if ok then 
                        args = ret
                    else
                        args = nil
				...
				send_request(inputArray[2], args)
			else
