...
function skynet.register_protocol(class)
	...
	local id = class.id
	if (proto[name] ~= nil or proto[id] ~= nil) then
		print("skynet.register_protocol name ,id ",name,id,proto[name])	
	end		
	-- 可以更新proto[name] = class
	-- print("skynet.register_protocol name ,id ",name,id,proto[name])	
...
if (skynet.getenv "vscdbg_open" == "on") then
	loghelper = function(level, flag, depth)
...
function skynet.make_logger(level, depth)
	local f = logLevel2Flag[level]
	return loghelper(level, f, depth)
end

local logd = skynet.make_logger(eloglevel.debug, 2)
local logw = skynet.make_logger(eloglevel.warn, 2)
local logi = skynet.make_logger(eloglevel.info, 2)
local loge = skynet.make_logger(eloglevel.error, 2)
